...
class CarRentalApp(App):
    def build(self):
        ...
        # Input field for rental duration
        self.rental_duration_input = TextInput(hint_text="Enter rental duration (in days)", multiline=False)
        self.layout.add_widget(self.rental_duration_input)
    ...
    def rent_car(self, instance):
        ...
        rental_duration = self.rental_duration_input.text
        if car_id.isdigit() and rental_duration.isdigit():
            car_id = int(car_id)
            ...
            for car in cars:
                if car["id"] == car_id:
                    if car["available"]:
                        ...
                        self.show_alert(f"Car rented for {rental_duration} days!")
                        return
                    else:
            ...
            self.show_alert("Please enter a valid Car ID and rental duration.")
